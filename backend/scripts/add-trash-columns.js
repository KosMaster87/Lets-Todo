...
async function addTrashColumns() {
  ...
  try {
    ...
    for (const db of databases) {
      ...
      if (tables.length === 0) {
        console.log(`⏭ No todos table in ${dbName}, skipping`);
        continue;
      }
      ...
      if (!hasTrashColumn) {
        ...
        console.log(`⏭ 'trashed' column already exists in ${dbName}`);
      }
      ...
      if (!hasTrashedAtColumn) {
        ...
        console.log(`⏭ 'trashed_at' column already exists in ${dbName}`);
      }
      ...
      try {
        ...
        if (indexError.code === "ER_DUP_KEYNAME") {
          console.log(`⏭ Index already exists in ${dbName}`);
        } else {
          console.warn(`Could not create index in ${dbName}:`, indexError.message);
        }
      }
