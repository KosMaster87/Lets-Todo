...
export const DataMaintenance = {
  /**
   * Clears all stored data across all storage types
   */
  clearAll() {
    try {
  ...
  /**
   * Clears user-specific data while preserving guest data
   */
  clearUserData() {
    try {
      StorageManager.removeData("session", StorageKeys.SESSION.AUTH_DATA);
   ...
   * @param {Array} todos - Todos array to repair
   * @param {Array} trashedTodos - Trashed todos array to repair
   * @returns {Object} Object with repaired arrays and hasChanges flag
   */
  repairTodos(todos, trashedTodos) {
    ...
    const repairedTrashedTodos = trashedTodos.map((todo) => {
      if (!todo.id) {
        hasChanges = true;
        return {
          ...todo,
    ...
    return {
      todos: repairedTodos,
      trashedTodos: repairedTrashedTodos,
      hasChanges,
    };
  },
};
